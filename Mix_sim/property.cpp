...
void tryAllOrderAction(string instrumentID){
    ...
    for(unordered_map<string, OriginalOrderFieldInfo*>::iterator it = originalOrderMap.begin();it!=originalOrderMap.end();it++){
        ...
        if(instrumentID==""){
            ...
            ///replace
            //ptradeApi->reqOrderAction(&clOrder);
            SimTradeData* stdac=new SimTradeData();
            stdac->clientOrderToken=oriOrderField->clientOrderToken;
            stdac->userID=oriOrderField->investorID;
            generateActionSimData(stdac);
...
void addNewOrderTrade(string instrumentID,string direction,string offsetFlag,double orderInsertPrice,int volume,string mkType, AdditionOrderInfo* addinfo){
    ...
    ///delete
    /// simdata
    SimTradeData* stda=new SimTradeData();
    stda->clientOrderToken = newOrderToken;
    stda->userID = boost::lexical_cast<string>(orderInfo->userID);
    stda->marketOrderToken = mkToken++;
    stda->mSide = orderInfo->m_Side;
    strcpy(stda->symbol, orderField.m_Symbol);
    stda->EES_OrderState=1;
    string ordersysid=boost::lexical_cast<string>(stda->marketOrderToken);
    strcpy(stda->EES_MarketOrderId, ordersysid.c_str());
    stda->orderInsertPrice = orderInfo->price;
    stda->tradePrice = orderInfo->price;
    stda->tradeVolume = orderInfo->volume;
    stda->volumeTotal = orderInfo->volume;
    generateTradeSimData(stda);
}
void generateActionSimData(SimTradeData* simdata){
    EES_OrderCxled* pOrder=new EES_OrderCxled();
    pOrder->m_ClientOrderToken=simdata->clientOrderToken;
    pOrder->m_Userid = boost::lexical_cast<int>(simdata->userID);
    ptradeApi->OnOrderCxledSim(pOrder);
}
...
void generateTradeSimData(SimTradeData* simdata){
    EES_OrderAcceptField* pAccept=new EES_OrderAcceptField();
    //memset(pAccept,0,sizeof(EES_OrderAcceptField));
    pAccept->m_ClientOrderToken = simdata->clientOrderToken;
    pAccept->m_MarketOrderToken = simdata->marketOrderToken;
    pAccept->m_OrderState = simdata->EES_OrderState;
    pAccept->m_Price = simdata->orderInsertPrice;
    pAccept->m_Qty = simdata->volumeTotal;
    pAccept->m_Side = simdata->mSide;
    strcpy(pAccept->m_Symbol,simdata->symbol);
    pAccept->m_UserID = boost::lexical_cast<int>(simdata->userID);
    ptradeApi->OnOrderAcceptSim(pAccept);
    ///
    EES_OrderMarketAcceptField* mkpAccept=new EES_OrderMarketAcceptField();
    //memset(mkpAccept,0,sizeof(EES_OrderMarketAcceptField));
    mkpAccept->m_MarketOrderToken = simdata->marketOrderToken;
    strcpy(mkpAccept->m_MarketOrderId,simdata->EES_MarketOrderId);
    strcpy(mkpAccept->m_Account,simdata->userID.c_str());
    ptradeApi->OnOrderMarketAcceptSim(mkpAccept);
    ////
    EES_OrderExecutionField* pExec=new EES_OrderExecutionField();
    //memset(pExec,0,sizeof(EES_OrderExecutionField));
    pExec->m_ClientOrderToken = simdata->clientOrderToken;
    pExec->m_MarketOrderToken = simdata->marketOrderToken;
    pExec->m_Price = simdata->tradePrice;
    pExec->m_Quantity = simdata->tradeVolume;
    pExec->m_Userid = boost::lexical_cast<int>(simdata->userID);
    ptradeApi->OnOrderExecutionSim(pExec);
}
...
void setRealInfo(double lastPrice){
    techCls.storageRealInfo ="mainDirection="+techCls.mainDirection+";ma5="+boost::lexical_cast<string>(techCls.trueKData15M->ma5)+";lastPrice="+boost::lexical_cast<string>(lastPrice);
}
...
void* lookbacktest(void* argv){
    for(int ri=0;ri<=0;ri++){
        techCls.relockTickNums=ri;
        for(int twograde=7;twograde<=10;twograde++){
            techCls.twoGrade=twograde;
            for(int tgap=1;tgap<=3;tgap++){
                techCls.twoGap=tgap;
                techCls.oneNormalGap=tgap;
                for(int nt=2;nt<=6;nt++){
                    techCls.nJumpTriggerSL=nt;
                    for(int ptime=1;ptime<=12;ptime++){
                        techCls.periodMinOne=ptime*3;
                        beginLookbackTest();
                    }
                }
...
void beginLookbackTest(){
    //开始时间
    boost::posix_time::ptime startTime = getCurrentTimeByBoost();
    //int lgh=sizeof(allMk2) / sizeof(allMk2[0]);
    /*
    for(int i=0;i<mkAmount-1;i++){
        MarketData* marketdatainfo=allMk2[i];
    ...
    double realPL=techCls.totalLockoverFlowPL+techCls.totalUnlockPL+techCls.totalHopePL+techCls.totalStopPL+techCls.emergePL;
    cout<<"avgTime="<<totalTime/(mkAmount-1)<<";process amount="<<(mkAmount-1)<<endl;
    string pl="realPL="+boost::lexical_cast<string>(realPL)+ ";lockOverFlow="+boost::lexical_cast<string>(techCls.lockOverFlow)+";totalLockoverFlowPL="+boost::lexical_cast<string>(techCls.totalLockoverFlowPL)
            +";unlock="+boost::lexical_cast<string>(techCls.unlock)+";totalUnlockPL="+boost::lexical_cast<string>(techCls.totalUnlockPL)
            +";emerge="+boost::lexical_cast<string>(techCls.emerge)+";emergePL="+boost::lexical_cast<string>(techCls.emergePL)
            +";hope="+boost::lexical_cast<string>(techCls.hope)+";totalHopePL="+boost::lexical_cast<string>(techCls.totalHopePL)
            +";stageTick3="+boost::lexical_cast<string>(techCls.stageTick3)+";totalStageTick3PL="+boost::lexical_cast<string>(techCls.totalStageTick3PL)
            +";stageTick2="+boost::lexical_cast<string>(techCls.stageTick2)+";totalStageTick2PL="+boost::lexical_cast<string>(techCls.totalStageTick2PL)
            +";stageTick1="+boost::lexical_cast<string>(techCls.stageTick1)+";totalStageTick1PL="+boost::lexical_cast<string>(techCls.totalStageTick1PL)
            +";totalStopPL="+boost::lexical_cast<string>(techCls.totalStopPL);
    string msg="lookbackresult;relockTickNums="+boost::lexical_cast<string>(techCls.relockTickNums)+";gap="+boost::lexical_cast<string>(techCls.twoGap)+";twoGrade="+boost::lexical_cast<string>(techCls.twoGrade)
            +";minite="+boost::lexical_cast<string>(techCls.periodMinOne)+";njump="+boost::lexical_cast<string>(techCls.nJumpTriggerSL)
            +";"+pl;
    cout<<msg<<endl;
    LOG(INFO)<<msg;
    reInitStrategyPara();
