...
void TraderDemo::reqOrderInsert(EES_EnterOrderField* orderField,AdditionOrderInfo* aoi){
    ...
    try {
        ///replace
        RESULT iResult = 0;//m_tradeApi->EnterOrder(orderField);
...
void TraderDemo::OnOrderAcceptSim(EES_OrderAcceptField* pAccept){
    //报单结构体
    OriginalOrderFieldInfo* oriOrderField;
...
void TraderDemo::OnOrderMarketAcceptSim(EES_OrderMarketAcceptField* pAccept)
{
    boost::recursive_mutex::scoped_lock SLock4(unique_mtx);//锁定
...
void TraderDemo::OnOrderExecutionSim(EES_OrderExecutionField* pExec)
{
    boost::recursive_mutex::scoped_lock SLock4(unique_mtx);//锁定
...
void TraderDemo::OnOrderCxledSim(EES_OrderCxled* pOrder){
    boost::recursive_mutex::scoped_lock SLock4(unique_mtx);//锁定
    //开始时间
