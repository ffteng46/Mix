            ...
            if(techCls.priceStatus=="0"){//just first open happen.
                ...
                if(false){//
                    ...
                    if(lastPrice >= (simFirstOpenPrice-techCls.oneSweetGrade*techCls.oneSweetGap*tickPrice - techCls.sweetToNormalTickNums*tickPrice)){
                        ...
                        LOG(INFO)<<"long逆向加仓处于第一阈值内,each grade加仓量="+boost::lexical_cast<string>(techCls.oneNormalVolume)+";oneNormalGap="+boost::lexical_cast<string>(techCls.oneNormalGap);
                        if(wfc_firstOpen&&int((wfc_firstOpen->openPrice-lastPrice)/tickPrice)%techCls.oneNormalGap==0){
                            LOG(INFO)<<"this price is at one normal gap,judge if add new order.lastPrice="+boost::lexical_cast<string>(lastPrice)+";openPrice="+
                                       boost::lexical_cast<string>(wfc_firstOpen->openPrice);
                            if(lastPrice < wfc_lastOpen->openPrice){
